...
// The id vocabulary.
//
// The settings bar sends buffs and consumables as strings, and an id
// this module cannot map is an error rather than a silent drop, so the
// web lane has to know exactly which strings exist. They are not a
// hand-kept list - they are read off the engine's own protobuf
// descriptors - so the list is generated here rather than written down
// twice: KnownBuffs and KnownConsumables are the vocabulary, IDS.md is
// the same vocabulary as prose for whoever builds the settings bar, and
// a test holds the committed file to what this code produces.
