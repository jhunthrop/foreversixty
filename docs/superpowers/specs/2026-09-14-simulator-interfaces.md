...
- **Saving is not running.** `POST /v1/sims` stores a browser result whose `engine_version` is any engine the site has shipped; it reads back with `stale` set when it is not the current one. Only a run request (the server lane, the jobs) must match `enginever.Version`. A result with `aborted: true` is never saved.
