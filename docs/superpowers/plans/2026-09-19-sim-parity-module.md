...
- Section 10, **Amendments after planning**, is the ruling on every gap the six lane plans found. It is binding and it wins over sections 1–9. Read it before Task 1 and re-read it whenever an earlier section seems to say something different.
- Where the contract is still silent, **add the name to the contract in its own commit first** (that is the contract's own rule, stated in its preamble) and then code against it. No task in this plan does; if you need one, stop and report it.
...
- `StepIterations = 1000`. `LaneIterationCeiling = {browser: 30000, server: 100000}`. `Caps = {browser: 400, server: 5000}` (contract A2: 20,000 cannot finish in the job's 15-minute timeout at the measured native rate). `measure.NativeIterationsPerCPUSecond = 1218`.
- Lanes are `api.LaneBrowser = "browser"` and `api.LaneServer = "server"`; they already exist.
- Origins: `equipped`, `bag`, `bank`, `search`, `drop:<source-id>`, `set:<name>`. A candidate also carries `SourceName`, the human name of that place, and the substitution copies it.
- Fight length is 20 s to 600 s (`MinDurationSec = 20`, `MaxDurationSec = 600`, widened from 60/480 by contract A3).
...
- The engine pin is `sim/enginever/version.go`'s `Version`, **`edc0c8e9a`** before Task 1. Never type a sha anywhere else; `make engine-pin` is the only writer of that file.
- The wasm exports are JSON strings in and out, failures `{"error": "..."}`. After this lane there are ten: `simRun`, `simSplit`, `simCombine`, `simAbort`, `simPlan`, `simRank`, `simCount`, `simNeedsMore`, `simValidate`, `simWeights`, plus the `simEngineVersion` string global and the `wasmready()` handshake. A cap breach from `simPlan` or `simCount` is the structured `{"error":"cap_exceeded","cap":n,"combinations":n}`.
- **A bulk stage's requests run unsplit.** `simSplit` and `simCombine` are for plain runs; a stage is already many independent sims and the pool runs them whole, in chunks of its width, which is what makes "abort returns the partial" true.
- `forever-sim` flags are `-in -out -out-proto -iterations -progress -version`, plus `-plan` (print the combination count and the first stage, run nothing) added by this lane; exit codes stay `0` ok, `1` sim failure, `2` bad args/input, `130` aborted.
...
| `sim/request/cooldowns.go` | `CooldownSpec` → `proto.Cooldowns`. |
...
| `sim/bulk/options.go` | `Options` and the embedded enchant table it defaults to. |
| `sim/internal/simdb/enchants.go` | The build's enchant table, embedded by `make simdb`. |
| `sim/internal/simdb/enchants_test.go` | Loader and fit rules. |
...
| `sim/measure/native.go` | `NativeIterationsPerCPUSecond` and the job's iteration budget. |
| `sim/measure/native_test.go` | The server cap fits the budget. |
| `sim/cmd/wasm/exports.go` | The JSON bodies of every export, host-testable (no build tag). |
| `sim/cmd/forever-sim/bulk.go` | The native plan–rank loop and its stage progress lines. |
| `sim/cmd/forever-sim/bulk_test.go` | The loop end to end, and `-plan`. |
| `sim/cmd/forever-sim/target.go` | The Smart Sim loop and the native weights run. |
| `sim/cmd/forever-sim/target_test.go` | Stepping, the ceiling, and weights. |
| `sim/internal/simdb/testdata/enchants.excerpt.json` | Ten-row excerpt of a build's enchant table. |

**Modified files**

| File | Change |
| --- | --- |
| `sim/api/envelope.go` | `Bulk`, `Weights`, `TargetError`, `NoSample`, the 20–600 s duration range, encounter additions, `CooldownSpec`, result additions, `Progress` additions, `ValidateLane`, `NeedsMoreIterations`. |
| `sim/request/request.go` | `encounter()` gains movement, targets-over-time, dummy, level, armor, type; `Player.Cooldowns`; `Options.NoSampleIteration`. |
...
| `sim/runner/runner.go`, `sim/runner/native.go`, `sim/runner/fixture.go` | `StageProgress` and `StageRunner` beside the unchanged `Progress` and `Runner`. |
| `sim/cmd/wasm/main.go` | The six new exports' `js.Func` wrappers and the weights engine call. |
| `sim/cmd/forever-sim/main.go` | Kind dispatch: bulk loop, weights, Smart Sim loop; the `-plan` flag. |
| `Makefile` | `simdb` carries the enchant table too; `sim-styles`, `styles-check`. |
| `.github/workflows/sim.yml` | Style check, enchant-table path, four-candidate Top Gear smoke over the ten exports. |
| `.gitignore` | The copied `sim/internal/simdb/enchants.json`. |

---

## The rulings this plan is written against

The contract's section 10 answers every gap the six lane plans found. The ones that changed this lane, and where they land:

| Ruling | Where |
| --- | --- |
| A1 lane-aware caps — `ValidateLane` | Task 3 |
| A2 server cap 5,000, `measure.NativeIterationsPerCPUSecond` | Task 2 |
| A3 iteration rules by kind, duration 20–600 s | Tasks 2, 3, 5 |
| A4 mode decides expansion; no `combinations` boolean | Tasks 2, 14 |
| A5 `BulkSpec.Consumables` as a candidate dimension | Tasks 2, 14 |
| A6 `Candidate.SourceName`, item names on `Substitution` | Tasks 2, 6, 13 |
| A7 `Reference` required; the Stats section generated from the enum | Tasks 4, 10 |
| A8 `TargetArmorByLevel` = 3300 / 3444 / 3588 / 3731 | Task 5 |
| A9 `ExpandWith`; enchants embedded by `make simdb` | Tasks 12, 13 |
| A10 `StageRequests.Ran` | Tasks 15, 17 |
| A11 progress widening is additive | Task 22 |
| A12 sample rows carry action keys, not names | Tasks 6, 19 |
| 10.2 `simCount`, `simNeedsMore`, `simValidate`, structured cap error, `-plan`, unsplit stages | Tasks 14, 20, 23, 25 |
| 10.3 `SimItem` 20–23, `SampleIteration`, opt-in, target padding, pin order | Tasks 1, 8, 19 |

---

### Task 1: Bump the engine pin and prove the new engine surface is there
...
**Interfaces:**
- Consumes: the contract's section 10 rulings (A2, A8, A12 and 10.3), which the engine and data lanes implement.
...
### Task 2: The envelope's kinds, bulk block, precision ladders and caps

**Files:**
- Create: `sim/api/bulk.go`
- Create: `sim/api/bulk_test.go`
- Create: `sim/measure/native.go`
- Create: `sim/measure/native_test.go`
...
- Produces:
  ...
  - `api.BulkSpec{Mode, Candidates, Talents, Sets, Consumables, Locked, Precision, Cap}`
  - `api.Candidate{Slot, ItemID, Enchant, Suffix, Origin, SourceName}`
  ...
  - `measure.NativeIterationsPerCPUSecond` — the constant the api lane's `too_large` estimate is built from
...
- [ ] **Step 6: Publish the native benchmark constant**

The server cap and the API's `too_large` refusal are two readings of one measurement, and the contract (A2) says both come from `sim/measure`. Create `sim/measure/native.go`:
...
and `sim/measure/native_test.go`:
...
`sim/measure` does not import `sim/api` today. If that import creates a cycle — it does not, `sim/api` imports only `logs/engine/summary` and `sim/enginever` — the test is fine as written; otherwise move the assertion into `sim/api/bulk_test.go` and import `sim/measure` there instead.

- [ ] **Step 7: Run the tests**

Run: `go test ./sim/api/... ./sim/request/... ./sim/measure/... -run 'Kind|Bulk|Ladder|CapExceeded|GearSlots|ServerCap|Budget' -v`
Expected: PASS.

- [ ] **Step 8: Run the whole api, request and measure packages, then vet and format**

Run: `go test ./sim/api/... ./sim/request/... ./sim/measure/...` then `go vet ./sim/api/... ./sim/request/... ./sim/measure/...` and `gofmt -l sim/api sim/request sim/measure`
Expected: PASS, no vet findings, no unformatted files.

- [ ] **Step 9: Commit**

```bash
git add sim/api/bulk.go sim/api/bulk_test.go sim/api/envelope.go sim/api/envelope_test.go sim/request/mapping_test.go sim/measure/native.go sim/measure/native_test.go
git commit -m "feat(sim): the envelope's bulk block, kinds, precision ladders and the measured lane caps
...
### Task 3: Smart Sim — target error, the lane ceilings, and the one place that decides "another step?"
...
**Interfaces:**
- Consumes: `api.SimRequest`, `api.SimResult`, `api.Estimate`, `api.Caps` (Task 2).
- Produces:
...
Add to `SimRequest`, after `Bulk`. (The `Weights` field is Task 4's, added beside the type it needs.)
...
Replace the iteration `switch` written in Task 2 with:
...
**Interfaces:**
- Consumes: `api.SimRequest`, `api.Kind()` (Task 2).
...
In `sim/api/envelope.go`, beside the bulk call added in Task 2:
...
### Task 5: The encounter's new fields and the character's cooldowns
...
- [ ] **Step 3: Widen the fight-length bounds**

Contract A3 widens the duration control to the range the style presets need — a dungeon pull's target timeline runs to 160 seconds and a target dummy has no natural end. In `sim/api/envelope.go`:
...
Add the two cases to `TestEncounterAdditionsValidation`'s table:
...
- [ ] **Step 4: Extend `EncounterSpec` and `CharacterSpec` in `sim/api/envelope.go`**
...
### Task 6: The result's new fields and the progress line's stage
...
**Interfaces:**
- Consumes: `api.Estimate`, `api.GearSlot`, `api.StatWeight` (Task 4).
- Produces:
  ...
  - `api.Substitution{Kind, Slot, ItemID, Enchant, Suffix, Name, Talents, Origin, SourceName, Consumes}` with `SubstitutionItem/Talents/Set/Consumes`; a `consumes` substitution's `Name` is its ids joined by `", "`
  - `api.Stage{Iterations, Combos}`
  - `api.SampleCast{AtMS, Action, Target, Resources}` — `Action` is the summary's action key, never a display name (contract A12)
...
### Task 7: The fight-style table, as a Go map and the JSON the page is tested against
...
**Interfaces:**
- Consumes: `api.EncounterSpec`, `api.Movement`, `api.TargetCount`, `api.DefaultEncounter()` (Task 5).
- Produces:
...
### Task 8: Map the new encounter fields onto the engine
...
**Interfaces:**
- Consumes: `api.EncounterSpec` additions (Task 5), the pinned engine's `proto.MovementPattern`, `proto.TargetCountAt`, `Encounter.TargetDummy` (Task 1).
...
### Task 9: Cooldown timings onto `proto.Cooldowns`
...
**Interfaces:**
- Consumes: `api.CooldownSpec` (Task 5), `request.Consumables`.
- Produces:
...
### Task 10: Graded buff ids, the world-buff section, and the stat vocabulary
...
- Produces:
  ...
  - `request.KnownStats() []string`, `func request.ParseStat(id string) (proto.Stat, bool)`, `request.statID` with the one spelling exception (`MP5` → `mp5`)
...
### Task 11: Item rows `sim/bulk` can read without touching a protobuf
...
**Interfaces:**
- Consumes: the embedded `proto.SimDatabase` and the four `SimItem` fields Task 1 pinned.
- Produces:
...
### Task 12: The enchant table, embedded beside the items

`SimEnchant` carries an effect id and a stat array and nothing about where an enchant may go, so enchant fit cannot come from the engine's own reduced rows. Contract A9 rules that the artifacts get enchants the way they get items: `make simdb` embeds the build's enchant table beside its item database, so `Expand` needs no file at runtime and the browser has nothing to fetch. `data/builds/<build>/enchants.json` (data lane) is the same rows, for the UI.

**Files:**
- Create: `sim/bulk/doc.go`
- Create: `sim/internal/simdb/enchants.go`
- Create: `sim/internal/simdb/enchants_test.go`
- Create: `sim/internal/simdb/testdata/enchants.excerpt.json`
- Create: `sim/bulk/options.go`
- Modify: `Makefile` (the `simdb` target copies the enchant table too)
- Modify: `.gitignore` (the copied `sim/internal/simdb/enchants.json`, beside `simdb.bin`)

**Interfaces:**
- Consumes: `simdb.Item` (Task 11).
- Produces:
  - `simdb.Enchant{ID, Name, Icon, Slots, ItemTypes, Classes, Phase}`
  - `func simdb.LookupEnchant(id int) (Enchant, bool)`, `func simdb.Enchants() (map[int]Enchant, error)`
  - `func simdb.EnchantFits(effectID int, slot string, item Item, class string) error`
  - `simdb.ErrUnknownEnchant`, `simdb.ErrEnchantDoesNotFit`
  - `bulk.Options{Enchants EnchantTable}` with `bulk.EnchantTable` an interface satisfied by the embedded table, so a test can pass its own

- [ ] **Step 1: Teach `make simdb` to carry the enchants**

In the `Makefile`, beside `SIMDB_EMBED`/`SIMDB_SRC`:
...
change the `simdb` target and its check:
...
and add the copy rule beside the existing one:
...
Add `sim/internal/simdb/enchants.json` to `.gitignore` beside `sim/internal/simdb/simdb.bin`.

- [ ] **Step 2: Write the test fixture**

`//go:embed` needs the real file at compile time, and the tests need a small one they can reason about. Create `sim/internal/simdb/testdata/enchants.excerpt.json` — ten rows in the contract's section 6.2 shape, which is also the shape of the copied file:
...
func excerpt(t *testing.T) map[int]Enchant {
	...
	if len(table) != 10 {
		t.Fatalf("the excerpt carries %d enchants, want 10", len(table))
	}
	return table
}
...
func TestEnchantErrorsSayWhy(t *testing.T) {
	err := fits(excerpt(t), 2568, "main_hand", Item{ID: 2, HandType: HandOne, WeaponType: "sword"}, "warrior")
	for _, want := range []string{"Superior Impact", "two_hand", "one_hand"} {
		if !strings.Contains(err.Error(), want) {
...
Run: `go test ./sim/internal/simdb/... -run 'Enchant' -v`
Expected: FAIL to compile — `undefined: Enchant`.

- [ ] **Step 5: Write `sim/internal/simdb/enchants.go`**
...
//
// It rides with the item database and for the same reason: sim/bulk
// has to know whether an enchant may go where a candidate is being
// sent, and the engine's own SimEnchant is an effect id and a stat
// array - the engine is TOLD where an enchant went rather than asked
// whether it could go there. The planner has to ask.
//
...
// data/builds/<build>/enchants.json here beside simdb.bin; the copy
// is git-ignored and the source is committed. The data lane publishes
// the same rows for the browser UI's own enchant picker.
...
type Enchant struct {
	...
	// derived by the data lane from the fork's type and extra_types.
	...
	// this package's: a hand type ("two_hand") or a weapon type
	// ("shield").
...
// parseEnchants decodes the table. It is separate from the embedded
// load so the tests can read a small fixture through the same code.
...
`slotIndexes` in the test is the package-local set of gear slot names `items.go` already derives for `Item.Slots`; if Task 11 did not leave a named set, add one there rather than a second list here:

```go
// slotIndexes is every gear slot name this package produces, so a
// table that names one it does not is caught rather than silently
// matching nothing.
var slotIndexes = func() map[string]bool {
	out := map[string]bool{}
	for _, slots := range slotsByType {
		for _, s := range slots {
			out[s] = true
		}
	}
	for _, slots := range slotsByHand {
		for _, s := range slots {
			out[s] = true
		}
	}
...
- [ ] **Step 7: Write `sim/bulk/options.go`**
...
```bash
make simdb
go test ./sim/internal/simdb/... -v
go build ./sim/bulk/...
```

Expected: `embedded data/builds/<build>/enchants.json (… bytes)`, then PASS with a log line naming the enchant counts. If `enchants.json` does not exist for the active build, **stop and report that the data lane has not published it** — Task 1's gate should have caught this.

- [ ] **Step 9: Vet, format, commit**

```bash
go vet ./sim/internal/simdb/... ./sim/bulk/...
gofmt -w sim/internal/simdb sim/bulk
git add sim/internal/simdb sim/bulk Makefile .gitignore
git commit -m "feat(sim): the build's enchant table, embedded beside the items
...
### Task 13: `Expand`, part one — placements, eligibility and enchant inheritance
...
**Interfaces:**
- Consumes: `api.SimRequest`, `api.BulkSpec`, `api.Candidate`, `api.GearSlot`, `api.Substitution` (Tasks 2, 7); `simdb.Lookup`, `simdb.Item` (Task 11); `bulk.Options`, `bulk.Enchants` (Task 12).
- Produces:
  ...
  - unexported `placement{Slot, Gear, Candidate}` and `placements(req, opt)` which Task 14 builds combinations from
...
`combinations` is Task 14; write it as a stub that returns one combination per placement so this task's tests can run, and replace it there:
...
and the shared applier, which Task 14 keeps:
...
### Task 14: `Expand`, part two — combination shapes, validity and the cap
...
**Interfaces:**
- Consumes: `placement`, `apply`, `baseGear` (Task 13); `api.ErrCapExceeded`, `api.Caps` (Task 2).
- Produces: the real `combinations`, enforcing one item per slot, two-hand versus main-plus-off-hand, duplicate rings and trinkets, same-name rings and trinkets, unique-equipped, the talents, sets and consumables dimensions, and the cap; `consumableChoice`; `bulk.Count`/`bulk.CountWith`.
...
`sim/internal/simdb` is already imported by `expand_test.go` (Task 13's import block); nothing new is needed.
...
- [ ] **Step 4: Add `Count`, which answers without building requests**

The page shows a live combination count as candidates are ticked, and the API refuses an over-cap request at submit; neither wants a slice of requests. Append to `sim/bulk/expand.go`:
...
and the tests, in `sim/bulk/expand_test.go`:
...
Expected: PASS, including Task 13's cases.
...
gofmt -w sim/bulk
git add sim/bulk
git commit -m "feat(sim): combination shapes, weapon validity, unique-equipped, the cap and Count
...
**Interfaces:**
- Consumes: `ExpandWith`, `Combination`, `Options` (Tasks 12–14); `api.Ladders`, `api.FinalIterations` (Task 2).
- Produces:
...
### Task 16: `Rank`, part one — scoring a stage and cutting to the next
...
**Interfaces:**
- Consumes: `StageRequests`, `stageRequests` (Task 15); `api.Ladder`, `api.Cut`, `api.Ladders` (Task 2).
- Produces:
...
`finalResult` is Task 17; write it as a stub that returns a result with only `Request` and `Equipped` set so this task's tests compile, and replace it there:
...
### Task 17: `Rank`, part two — the finished result, and the within-error groups
...
**Interfaces:**
- Consumes: `scored`, `Rank` (Task 16); `api.Combo`, `api.Stage`, `api.SimResult` (Task 6).
...
### Task 18: Stat weights — the engine request and the result mapping
...
**Interfaces:**
- Consumes: `api.WeightsSpec`, `api.StatWeight` (Task 4); `request.ParseStat` (Task 10); `request.BuildWith`.
- Produces:
...
### Task 19: The sample iteration

**Files:**
- Modify: `sim/request/request.go` (opt in for plain runs only)
- Modify: `sim/request/request_test.go`
- Modify: `sim/adapter/adapter.go`
- Modify: `sim/adapter/adapter_test.go`
...
**Interfaces:**
- Consumes: `proto.SampleIteration`, `proto.SampleCast`, `proto.SimOptions.SampleIteration` (contract 10.3, pinned by Task 1); `adapter.ActionName`.
- Produces: `func adapter.Sample(res *proto.RaidSimResult) []api.SampleCast`, `SimOptions.SampleIteration` set for plain runs only, and both lanes filling `SimResult.Sample`.

- [ ] **Step 1: Write the failing test for the opt-in**

The engine records the sample only when asked (`SimOptions.sample_iteration`, contract 10.3), and a bulk stage must never ask: sixteen cast logs per stage is a result blob nobody reads and a cost on every combination. Append to `sim/request/request_test.go`:
...
Run: `go test ./sim/request/... -run SampleIteration -v`
Expected: FAIL to compile — `unknown field NoSampleIteration`.

- [ ] **Step 3: Add the option and set the flag**

In `sim/request/request.go`, add to `Options`:
...
and in the `proto.SimOptions` literal in `BuildWith`:
...
- [ ] **Step 4: Set it from the planner**

`sim/bulk` does not call `request` — it produces envelopes, not protobufs — so the flag is set by whoever builds the stage's requests. Both do. In `sim/cmd/wasm/main.go`'s `simRun` and `sim/cmd/forever-sim/main.go`'s `execute`, the option block becomes:

```go
	request.Options{OpenIterations: true, NoSampleIteration: req.Bulk != nil || req.Kind() != api.KindRun}
```

That is wrong for a stage request, whose `Bulk` is nil — so instead put the decision where it is knowable. Add a field to the envelope in the same step, `sim/api/envelope.go`:
...
set it in `sim/bulk/plan.go`'s `stageRequests` on `equipped` and on every combination's request:
...
and read it in both lanes:

```go
	request.Options{OpenIterations: true, NoSampleIteration: req.NoSample}
```

Add the round-trip assertion to `sim/bulk/plan_test.go`'s `TestPlanIsTheFirstRungOfTheLadder`:

```go
				if !r.NoSample {
					t.Errorf("request %d asks for a sample iteration; a stage is dozens of runs", i)
				}
```

- [ ] **Step 5: Write the failing test for the adapter**
...
- [ ] **Step 7: Write `Sample` in `sim/adapter/adapter.go`**
...
- [ ] **Step 9: Say where a bulk result's sample comes from**

Add to `finalResult` in `sim/bulk/rank.go`, beside `out.IterationsRun`:

```go
	// No sample: every stage request sets NoSample, so results[0]
	// carries none and there is nothing to copy. Stated rather than
	// left implicit, because the obvious alternative - the winner's
	// cast log - would be a log of a character the player does not
	// have.
	out.Sample = nil
```
...
Run: `go test ./sim/request/... ./sim/adapter/... ./sim/bulk/... ./sim/cmd/forever-sim/... -v`
...
- [ ] **Step 11: Vet, format, commit**

```bash
go vet ./sim/api/... ./sim/request/... ./sim/adapter/... ./sim/bulk/... ./sim/cmd/...
gofmt -w sim/api sim/request sim/adapter sim/bulk sim/cmd
git add sim/api sim/request sim/adapter sim/bulk sim/cmd
git commit -m "feat(sim): the sample iteration's cast log, opted into for plain runs only
...
### Task 20: `simPlan`, `simRank`, `simCount`, `simNeedsMore` and `simValidate`
...
**Interfaces:**
- Consumes: `bulk.Plan`, `bulk.Rank`, `bulk.StageRequests`, `bulk.Count` (Tasks 14–17); `api.NeedsMoreIterations` (Task 3); `api.ErrCapExceeded` (Task 2).
- Produces:
  - `simPlan(requestJSON)` → a `StageRequests` JSON, or an error shape
  - `simRank(requestJSON, stageJSON, resultsJSON)` → `{"next": <stage>}` or `{"result": <SimResult>}`, or an error shape
  - `simCount(requestJSON)` → `{"combinations": n}`, or an error shape
  - `simNeedsMore(resultJSON, requestJSON)` → `{"needs_more": bool}`
  - `simValidate(requestJSON)` → `{"ok": bool, "errors": [{"field","message"}]}`
  - the structured cap error `{"error":"cap_exceeded","cap":n,"combinations":n}` from `simPlan` and `simCount`
...
Move `decodeRequest` and `errorJSON` out of `main.go` into this file unchanged, so both build tags see them. The new imports this file needs are `errors`, `regexp` and `strings` alongside `encoding/json`.
...
Update the package comment's "exactly four functions" to name all ten (`simRun`, `simSplit`, `simCombine`, `simAbort`, `simPlan`, `simRank`, `simCount`, `simNeedsMore`, `simValidate`, `simWeights` in Task 21), say that `simSplit`/`simCombine` are for plain runs only and that a bulk stage's requests run whole, and change the handshake sentence to say the host page is told when all of them exist.
...
**Interfaces:**
- Consumes: `request.BuildWeights` (Task 18), `simdb.AttachWeights` (Task 18), `adapter.Weights` (Task 18), `core.StatWeightsAsync`.
...
### Task 22: Give the runner a stage-aware progress callback

The contract's section 8 says progress rows carry `stage`, `combos_done` and `combos_total`. `sim/runner` is how the api module reads a native run's progress, and its callback carries two bare numbers.

**The widening is additive** (contract A11): `runner.Progress` keeps its signature and the `Runner` interface is unchanged, so the `api` module keeps compiling when the `sim` module merges first. A caller that wants the stage fields asks for them through a second, optional interface, which `Native` satisfies and which the api lane adopts on its own schedule.
...
**Interfaces:**
- Consumes: `api.Progress` (Task 6).
- Produces:
  - `type runner.Progress func(done int, mean float64)` — **unchanged**
  - `type runner.StageProgress func(p api.Progress)`
  - `type runner.StageRunner interface { Runner; RunStaged(ctx, req, onProgress StageProgress) (api.SimResult, error) }`
  - `Native` and `Fixture` implementing `StageRunner` and parsing `stage`, `combos_done`, `combos_total` off the tick line

- [ ] **Step 1: Write the failing test**

Leave every existing callback in `sim/runner/runner_test.go` as it is — that they still compile is half of what this task delivers — and add:
...
func TestNativeReportsStageProgress(t *testing.T) {
	...
	if _, err := n.RunStaged(context.Background(), validRequest(), func(p api.Progress) {
		seen = append(seen, p)
	}); err != nil {
		t.Fatal(err)
	}
...
func TestTheTwoCallbackShapesAgree(t *testing.T) {
	stderr := `{"completed":200,"total":400,"dps":950,"stage":1,"combos_done":2,"combos_total":4}`
	var done int
	var mean float64
	n := &Native{Binary: fakeBinary(t, resultJSON(t), stderr, 0)}
	if _, err := n.Run(context.Background(), validRequest(), func(d int, m float64) {
		done, mean = d, m
	}); err != nil {
		t.Fatal(err)
	}
...
Expected: FAIL to compile — `undefined: StageRunner`, `n.RunStaged undefined`.

- [ ] **Step 3: Add the second callback and the second interface in `sim/runner/runner.go`**
...
`Run` becomes a wrapper over `RunStaged`, so there is one implementation and the narrow callback cannot fall behind the wide one:
...
Rename today's `Run` body to `RunStaged` with the wider callback, and replace the `tick` struct and its use:
...
Give it the same pair: `RunStaged` with the body, `Run` as the narrowing wrapper. If the fixture runner reports a single mid-run tick, keep it at the same completion point and leave the three bulk fields zero.
...
- [ ] **Step 7: Check that nothing outside the module broke**

Run: `go build ./...` from the worktree root.
Expected: **everything still compiles, including the `api` module.** That is the deliverable: contract A11 says the widening is additive. If the api module fails, `Progress` was replaced rather than joined — go back to Step 3.
...
gofmt -w sim/runner
git add sim/runner
git commit -m "feat(sim): a stage-aware progress callback beside the plain one

A bulk run's progress is a stage and a combination count, not an
iteration count. StageProgress carries the whole api.Progress - the
same struct the browser's simProgress callback carries - and
StageRunner is the interface that offers it. Progress and Runner are
unchanged, so the api module keeps compiling and adopts the wider
callback on its own schedule (parity contract A11).
...
### Task 23: `forever-sim` runs the plan–rank loop
...
**Interfaces:**
- Consumes: `bulk.PlanWith`, `bulk.Rank`, `bulk.StageRequests`, `bulk.CountWith`, `bulk.Options` (Tasks 12–17); `Execute` (existing).
- Produces:
  - `func executeBulk(req api.SimRequest, opt bulk.Options, progress io.Writer) (api.SimResult, error)`
  - a `-plan` flag: print the combination count and the first stage, run nothing
...
- [ ] **Step 4: Dispatch on the kind in `main.go`, and add `-plan`**

The enchant table needs no flag: contract A9 embeds it beside the items, so `bulk.Options{}` is the right value here and always. What the binary does need is a way to answer "how many combinations, and what would the first stage be?" without running anything — the API counts combinations that way rather than importing `sim/internal` (contract 10.2).
...
func run(inPath, outPath string, iterations int, planOnly bool, progress io.Writer) error {
...
`ExecuteToTarget` and `executeWeights` are Task 24. Until then, make `dispatch`'s default arm `Execute(req, progress)` and its weights arm return an error saying so, and replace both there.

Update the two call sites of `run`/`runProto` in `main` to pass `*plan`, and give `runProto` the same parameter (ignored — it has one caller and that caller runs a plain sim). Update `main_test.go`'s calls to `run` for the new parameter.

Update the command's doc comment: the usage lines gain `forever-sim -plan -in request.json -out -`, and a paragraph says the binary detects the request's kind from the request itself and runs the plan–rank loop for a bulk one.

Add the test to `sim/cmd/forever-sim/bulk_test.go`:
...
func TestPlanOnlyRunsNothing(t *testing.T) {
	...
	if err := run(in, out, 0, true, nil); err != nil {
		t.Fatal(err)
	}
	var got struct {
		Combinations int `json:"combinations"`
...
### Task 24: `forever-sim` runs weights and the Smart Sim loop
...
**Interfaces:**
- Consumes: `api.NeedsMoreIterations`, `api.NextStepIterations` (Task 3); `combine.Results`; `request.BuildWeights`, `simdb.AttachWeights`, `adapter.Weights` (Task 18).
- Produces:
...
In `main.go`, replace Task 23's placeholder arms with the real calls: `api.KindWeights` → `executeWeights(req, progress)`, default → `ExecuteToTarget(req, progress)`.
...
### Task 25: The Makefile targets and the pipeline
...
**Interfaces:**
- Consumes: `go run ./internal/genstyles` (Task 7); the seven wasm exports (Tasks 20, 22).
...
Expected: the "ten exports … top gear: N ranked over 2 stages" line and exit 0. If the combination count is not 15, read the failure: it means one of the four slots' candidates was skipped, which is an eligibility bug and not a smoke-test bug.
...
**Order.** Task 1 is the gate: nothing else compiles until the fork and the data lane have landed their side of contract 10.3. Tasks 2–6 are the envelope; they touch one file, so run them in order. Tasks 7–10 (`sim/request`) and 11–17 (`simdb` and `sim/bulk`) are two lanes that do not touch the same files and can run in parallel once 2–6 are in. Tasks 18–21 need both. Tasks 22–24 need everything, and 25 is last.
...
**No cross-lane break.** Contract A11 makes the progress widening additive: Task 22 adds `StageProgress` and `StageRunner` beside the existing `Progress` and `Runner`, so the `api` module keeps compiling whichever module merges first, and the api lane adopts the wider callback when it is ready. If `go build ./...` from the worktree root ever fails in another module because of this lane, something was replaced that should have been added.

**What Task 1 depends on, exactly.** Contract 10.3 moves two pins together and in this order: `sim/enginever/version.go` (this lane, `make engine-pin`) and then `data/proto/ENGINE_SHA` with the vendored protos (data lane, `python -m pipeline genproto`). Task 1 does the first; it cannot finish until the data lane has done the second and re-run `python -m pipeline simdb`, because `make simdb` copies that output.
