...
- `report_id` is 12 lowercase base32 characters. `fight_index` is the engine's `fight.Fight.Index`; **the engine numbers fights from 1**, and this plan uses the engine's numbers verbatim (the contract's "0-based" line is wrong — see "Contract ambiguities resolved" at the end).
...
- Auth routes: `GET /v1/auth/battlenet/start?next=`, `POST /v1/auth/email {email}`, `GET /v1/me`, `POST /v1/devices/pair`, `GET /v1/devices`, `DELETE /v1/devices/{id}`.
- Upload: `POST /v1/uploads { size_bytes, filename }` → `{ upload_id, parts: [{ number, url }], complete_url }`, 64 MiB parts, 1-hour expiry; `POST /v1/uploads/{upload_id}/complete { etags: [{ number, etag }], title?, visibility }` → 202 `{ report_id }`.
- Rankings: `GET /v1/rankings?encounter=&difficulty=&metric=&spec=&class=&phase=&region=&ruleset=&faction=&page=`, `GET /v1/rankings/percentile?encounter=&difficulty=&spec=&phase=&metric=&value=`, `GET /v1/characters/{region}/{ruleset}/{name}`, `GET /v1/guilds/{region}/{ruleset}/{name}`, `GET /v1/rankings/guilds?encounter=&kind=&phase=`. A ranking row's guild is `{ name, ruleset, region }`.
...
**Two routes the contract does not name, resolved here** (the contract's own `/account` bullet requires both, and its `users.anonymize` column exists for the second): `DELETE /v1/sessions` signs out the current session and clears the cookie; `PATCH /v1/me { anonymize }` sets the read-time pseudonym flag from spec section 7. Both use the envelope and the CSRF header like every other state-changing call.
...
describe('the account API', () => {
  ...
  it('lists, pairs and revokes devices', async () => {
    ...
    const upstream = vi.fn<GlobalFetch>(async (input) => {
      ...
      if (request.method === 'POST') return envelope({ code: '4821-9930', expires_at: '2026-11-05T10:10:00Z' });
    ...
    await expect(pairDevice(API)).resolves.toEqual({ code: '4821-9930', expires_at: '2026-11-05T10:10:00Z' });
...
//
// Two routes are this plan's reading of the contract rather than lines in it, because the
// contract's /account bullet requires both and its users.anonymize column exists for the
// second: DELETE /v1/sessions and PATCH /v1/me { anonymize }.
import { API_BASE_URL } from '../planner/config';
...
{:else if mode === 'pairing'}
  <div class="flex flex-col gap-3" data-testid="pairing">
    ...
    {:else}
      ...
      <p class="text-muted text-[13px]">
        Type this into the companion within ten minutes. It pairs one device and cannot be reused.
      </p>
    {/if}
    {#if error !== ''}<p class="text-[14px]" role="alert" data-testid="account-error">{error}</p>{/if}
  </div>
  ...
  <div class="flex flex-col gap-8" data-testid="account">
    ...
    {:else}
      ...
      <section class="flex flex-col gap-3">
        ...
        {:else}
          <p class="font-mono tabular text-strong text-[24px]" data-testid="pairing-code">{pairing.code}</p>
          <p class="text-muted text-[13px]">Type this into the companion within ten minutes.</p>
        {/if}
      </section>
...
test('the account page lists devices, pairs one, and shows the character', async ({ page }) => {
  ...
  await page.route('**/v1/devices', (route) =>
    route.request().method() === 'POST'
      ? route.fulfill(fulfil({ ok: true, data: { code: '4821-9930', expires_at: '2026-11-05T10:10:00Z' }, error: null, request_id: 'r' }))
...
which the interface contract implies through its /account bullet and its
users.anonymize column without naming the routes.
...
**One route the contract does not name, resolved here:** the contract's `/logs` bullet requires a "Your reports" list and no route returns one. This plan uses `GET /v1/reports?mine=1&page=<n>` → envelope data `{ rows: [{ id, title, zone, status, visibility, created_at, fight_count, kill_count }], total, page, per_page: 50 }`, which the api plan implements.
...
- Produces: `MyReport`, `MyReportPage`, `listMyReports(page?: number, apiBase?: string): Promise<MyReportPage>` from `src/lib/account/api.ts`.
...
describe('listMyReports', () => {
  ...
  it('treats a signed-out visitor as an empty page rather than an error', async () => {
    vi.stubGlobal('fetch', vi.fn<GlobalFetch>(async () => envelope(null, 401)));
    const { listMyReports } = await import('./api');
    await expect(listMyReports(1, API)).resolves.toEqual({ rows: [], total: 0, page: 1, per_page: 50 });
  });
});
```
...
const EMPTY_REPORT_PAGE: MyReportPage = { rows: [], total: 0, page: 1, per_page: 50 };

/**
 * The contract requires a "Your reports" list on /logs without naming a route; this is the
 * one the api plan implements. A signed-out visitor gets an empty page rather than an
 * error, because /logs renders for them too -- it just tells them to sign in.
 */
...
<script lang="ts">
  ...
  const hasMore = $derived(rows.length > 0 && page * 50 < total);
</script>
...
The reports you own come from GET /v1/reports?mine=1, a route the contract's
own /logs bullet requires without naming; a signed-out visitor gets an empty
page instead of an error, because /logs renders for them too." -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>
Claude-Session: https://claude.ai/code/session_01EkkERdonhxS2PMgXD7qZ6k"
```
...
/**
 * The web-side half of the spec's "size ceiling". The API enforces its own; this one is
 * here so a mistaken 40 GB file is refused before a signed-url request is made.
 */
export const MAX_UPLOAD_BYTES = 2 * 1024 * 1024 * 1024;

export const UPLOAD_TOO_LARGE = 'That file is over 2 GB. Split the night or log it live instead.';
...
test('a log uploads part by part and hands off to the report', async ({ page }) => {
  ...
  await page.route('**/v1/reports?mine=1**', (route) => route.fulfill(fulfil({ rows: [], total: 0, page: 1, per_page: 50 })));
...
test('the pairing code and the companion downloads are on the page', async ({ page }) => {
  ...
  await page.route('**/v1/reports?mine=1**', (route) => route.fulfill(fulfil({ rows: [], total: 0, page: 1, per_page: 50 })));
  await page.route('**/v1/devices/pair', (route) => route.fulfill(fulfil({ code: '4821-9930', expires_at: '2026-11-05T10:10:00Z' })));
...
- Produces from `api.ts`: `RankingRow`, `RankingsPage`, `GuildRankingRow`, `CharacterPage`, `GuildPage`, `RankingsQuery`, `RankingsError`, `RANKINGS_FAILED`, `fetchRankings(query, apiBase?)`, `fetchGuildRankings(query, apiBase?)`, `fetchCharacter(path, apiBase?)`, `fetchGuild(path, apiBase?)`. Tasks 19 and 20 consume all of it.
...
export interface CharacterPage {
  ...
  builds_seen: { build_id: string; first_seen: string }[];
}

export interface GuildPage {
  guild: { name: string; region: string; ruleset: string };
  progression: { encounter: string; kills: number; first_kill_at: string | null; pull_count: number }[];
  roster_best: { name: string; class?: string; spec?: string; metric: string; value: number; encounter: string }[];
  ...
  {:else if state.board === 'guild'}
    ...
    {:else}
      <ul class="flex flex-col" data-testid="guild-rows">
        {#each guildRows as row (`${row.guild.region}-${row.guild.ruleset}-${row.guild.name}`)}
...
test('the guild board asks the guild endpoint', async ({ page }) => {
  ...
  await page.route('**/v1/rankings/guilds?**', (route) => {
    ...
    return route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ ok: true, data: { rows: [{ rank: 1, guild: { name: 'The Last Watch', ruleset: 'hardcore', region: 'us' }, value: 9, fought_at: '2026-12-09T22:10:00Z', report_id: 'fixture2abcd', state: 'ok' }] }, error: null, request_id: 'r' }),
    });
  });
    ...
    <section class="flex flex-col gap-2">
      ...
      {:else}
        <ul class="flex flex-col" data-testid="character-best">
          {#each data.best as row (`${row.encounter_id}-${row.metric}`)}
    ...
    {#if data.builds_seen.length > 0}
      <section class="flex flex-col gap-2">
        ...
        <ul class="flex flex-col" data-testid="character-builds">
          {#each data.builds_seen as build (build.build_id)}
            <li class="border-line-soft flex min-h-11 items-center gap-3 border-b px-2 py-2 text-[14px]">
              <a href={`/b/${build.build_id}`}>{build.build_id}</a>
...
<script lang="ts">
  import { parseGuildPath, rulesetLabel, splitUnitName, type CharacterPath } from '../lib/characters';
  import { classColorVar, formatAmount } from '../lib/report/format';
          ...
          {#each data.progression as row (row.encounter)}
            <li class="border-line-soft grid min-h-11 grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 border-b px-2 py-2 text-[14px]">
              ...
              <span class="font-mono tabular w-[104px] text-right" data-testid="guild-kill">
                {row.first_kill_at === null ? 'not killed' : row.first_kill_at.slice(0, 10)}
              </span>
            </li>
          {/each}
        </ul>
    ...
    {#if data.roster_best.length > 0}
      <section class="flex flex-col gap-2">
        ...
        <ul class="flex flex-col" data-testid="guild-roster">
          {#each data.roster_best as row, index (`${row.name}-${row.encounter}-${index}`)}
            <li class="border-line-soft grid min-h-11 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 border-b px-2 py-2 text-[14px] md:grid-cols-[minmax(120px,1fr)_minmax(0,1fr)_96px]">
              ...
              <span class="text-muted truncate hidden text-[13px] md:inline">{row.encounter}</span>
              <span class="font-mono tabular text-right">{formatAmount(Math.round(row.value))}</span>
            </li>
          {/each}
        </ul>
...
const CHARACTER = {
  ...
  data: {
    ...
    best: [{ encounter: 'Warden Kelthas', encounter_id: 9001, metric: 'hps', value: 1840, percentile: 96.2, report_id: 'fixture2abcd', fight_index: 3, fought_at: '2026-12-09T22:10:00Z' }],
    history: [{ encounter: 'Warden Kelthas', metric: 'hps', value: 1840, percentile: 96.2, fought_at: '2026-12-09T22:10:00Z', report_id: 'fixture2abcd', fight_index: 3, spec: 'Discipline' }],
    builds_seen: [{ build_id: 'k7x2qm4a', first_seen: '2026-12-09T22:10:00Z' }],
  },
  error: null,
  request_id: 'r',
};
...
const GUILD = {
  ...
  data: {
    ...
    progression: [
      { encounter: 'Warden Kelthas', kills: 2, first_kill_at: '2026-12-09T22:10:00Z', pull_count: 14 },
      { encounter: 'Deep Warden', kills: 0, first_kill_at: null, pull_count: 31 },
    ],
    roster_best: [{ name: 'Elyra Duskvale', class: 'Priest', spec: 'Discipline', metric: 'hps', value: 1840, encounter: 'Warden Kelthas' }],
...
test('a character page shows bests, history and the builds they were seen in', async ({ page }) => {
  ...
  await expect(page.getByTestId('character-builds').getByRole('link')).toHaveAttribute('href', '/b/k7x2qm4a');
});
...
progression is looking for." -m "Co-Authored-By: Claude Fable 5.1 <noreply@anthropic.com>
Claude-Session: https://claude.ai/code/session_01EkkERdonhxS2PMgXD7qZ6k"
```
...
|---|---|---|
| 1 | `fight_index` is "0-based" | The engine numbers fights **from 1** (`fight.Fight.Index`; verified against `logs/engine/event/testdata/v16.log`, which yields 1, 2, 3), and the same line says the engine's field is the contract. This plan uses the engine's numbers everywhere, and `defaultState(firstFight)` takes the first entry of `report.json` rather than assuming a number. (Task 2) |
| 2 | `/logs` needs a "Your reports" list | No route returns one. This plan uses `GET /v1/reports?mine=1&page=<n>` → `{ rows: [{ id, title, zone, status, visibility, created_at, fight_count, kill_count }], total, page, per_page: 50 }`. (Task 7) |
| 3 | `/account` needs sign out and an anonymize toggle | Neither route is named, though `users.anonymize` is in the schema. This plan uses `DELETE /v1/sessions` and `PATCH /v1/me { anonymize }`, both enveloped and CSRF-protected like every other state-changing call. (Task 6) |
