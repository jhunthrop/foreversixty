...
- **Worktree.** Every change in this plan is made in `/Users/jh/code/forever/.worktrees/sim-api` on branch `sim-api`, rebased onto `main` `ed49176`. Nothing outside that worktree is touched. `/Users/jh/code/wowsims-forever` (fork at `9326fffd9`) and the other lanes' worktrees are read-only references.
...
- **Every existing test must still pass.** Baseline in this worktree (`main` `ed49176`, engine lane `4166eb0`, re-pinned at dispatch): `go test -p 1 ./...` in `api/` is green, with `internal/site`'s built-site snapshot test skipping because `web/dist` is not built here. Nothing in this plan may turn a green package red.
...
**An unmeasured card has no date, and says so.** `UpdatedAt` is a `*time.Time`, not a `time.Time`: a card the nightly job has never written has never been updated, and a `time.Time` zero value would ship `"0001-01-01T00:00:00Z"` — a real-looking date from the year one, which a page would happily render as "measured 2,025 years ago". Null is the honest answer, it is the same shape `median_gap` already uses for the same reason, and the test below pins it.
    ...
    SpecFidelity:
      ...
      properties:
        ...
        updated_at:
          ...
          format: date-time
          nullable: true
          description: Null until the nightly job has measured this spec.
```
...
- Modify: `api/internal/reports/ingest.go`, `api/internal/reports/ingest_test.go` (two new fakes, one helper and four new tests; nothing existing changes), `api/internal/auth/store.go` (one read), `api/internal/auth/store_test.go` (one test), `api/internal/server/sims_test.go` (the wiring test, Step 10), `api/cmd/api/main.go`
- Test: `api/internal/sims/score_test.go`, `api/internal/reports/ingest_test.go`, `api/internal/auth/store_test.go`, `api/internal/server/sims_test.go`
...
`FightCharacter` returns every reason at once rather than the first, so a log line names the whole gap; the race reason is always among them today, which is why `Score` always skips.
...
**Cast counts are compared by row identity, not by name.** `adapter.ActionName` (engine lane, as of `4166eb0`) gives every summary row a unique integer id: a plain untagged spell keeps its client spell id, and a tagged, item or `OtherId` action gets a reserved id above 2,000,000. A sim's `CastRow.SpellName` is the engine's own `"spell:23881"` form while a real fight's is `"Bloodthirst"`, so the two sides are joined on `SpellID` — which lines up exactly for the plain spells that make up a rotation — and the display name comes from whichever side has a human one.
...
Checked every name that crosses a task or a lane boundary, against the files as of `sim-engine` `4166eb0`, `main` `ed49176`, and the fork at `9326fffd9`.
...
- `SpecFidelity.MedianGap` and `SpecFidelity.UpdatedAt` are pointers everywhere, so "unmeasured" is null and never a zero gap or a year-one date; `ExecutionScore` is `*float64` on all three row types (Task 9), so "no score" is null everywhere it appears.
