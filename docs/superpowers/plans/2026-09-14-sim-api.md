...
1. `sim/api`, `sim/enginever` and `sim/specs` exist at the repository root of **this worktree**. They arrive by rebasing `sim-api` onto a `main` that carries the engine lane's Tasks 2–3 and the data lane's spec generation. As of 2026-09-18 they are on `sim-engine` (`6659972`, whose Task 3 fix round is the state this plan is written against) and `sim-data` (`c0fcd65`) and **not yet on `main`**.
...
2. **Level.** Not recorded either; the engine simulates exactly one level, `simapi.SimLevel` (60), and as of `6659972` the envelope's own `Validate` is the only thing that says so, so this one has a safe answer.
...
`data/builds/<build>/simconsumes.json` (engine lane `6659972`,
`request.BuildWith` with `Options{Consumables}`). The image already
carries `data/builds` at `/data`, so the table is there; how the binary
is pointed at it is the engine lane's flag surface, and the deploy
notes follow it once `sim/cmd/forever-sim` exists.
```
...
- The first draft compared the sim's cast counts with a parse's **by name**. Since `adapter.ActionName` names a sim's rows `"spell:23881"` and a log names the same row `"Bloodthirst"`, that join would have matched nothing and every ability would have shown a hundred per cent gap. Both sides now key on the row identity `ActionName` returns (Tasks 6 and 13), which is also what keeps a summary's rows unique.
...
Checked every name that crosses a task or a lane boundary, against the files as of `sim-engine` `6659972` and `sim-data` `c0fcd65`.
...
- Row identity: `WorstAction.SpellID` and `CastCount` (Tasks 6, 13) carry the integer `adapter.ActionName` returns, and both sides of the cast-count comparison are keyed by it. No code in this lane joins summary rows by name.
- The level rule lives in one place, `simapi.SimLevel` and the envelope's `Validate`; `request.ErrUnsupportedLevel` no longer exists and is cited nowhere.
