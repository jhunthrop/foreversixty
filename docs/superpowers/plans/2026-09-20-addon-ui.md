...
- Modify: `addon/ForeverSixty/Export.lua` (three helpers made public, the `savedAt` stamp, `Export.show` deleted)
- Modify: `addon/ForeverSixty/Options.lua` (the `export` branch uses `Export.string`)
- Modify: `addon/tests/export_spec.lua` (one new example for the stamp)
- Modify: `addon/ForeverSixty/Locale.lua` (eleven Export-tab strings, one reworded)
...
Spec 3.2's "A level-8 character with no talents … still exports" is **not** implemented: `Export.string` keeps refusing a character with no points spent, because removing that refusal changes the FS1 contract the site's decoder and the companion already ship against, which is another lane's code. The tab shows `L.exportNoTalents` in the warning colour with the box hidden — which is exactly spec 3.2's last bullet. This is flagged to the human in the Phase 1 report.
...
describe("ExportView", function()
	...
	it("says there are no talent points yet and carries the refusal as the reason", function()
		start({ talents = {
		...
		local model = ExportView.summary(DATA)
		assert.is_nil(model.code)
		assert.are.equal(L.exportNoTalents, model.reason)
	end)
	...
	it("hides the box and shows the reason when there is nothing to export", function()
		start({ talents = {
			{ name = "Arms", talents = { { name = "Improved Heroic Strike", tier = 1, column = 1, rank = 0, maxRank = 3 } } },
			{ name = "Fury", talents = {} },
			{ name = "Protection", talents = {} },
		...
		assert.are.equal(L.exportNoTalents, view.reason:GetText())
		assert.is_true(view.reason:IsShown())
	end)
end)
```

- [ ] **Step 2: Add the new example to `export_spec.lua`**

Append inside the existing `describe("Export", …)` block:
...
In `addon/ForeverSixty/Locale.lua`, replace the `exportNoTalents` line and add the rest of the Export block:

```lua
	exportNoTalents = "No talent points yet. Spend one and your talents go into the export.",
...
- [ ] **Step 7: Point `Options.handle`'s export branch at `Export.string`**
...
function ExportView.summary(data)
	...
	return {
		classSlug = classSlug,
		trees = trees,
		talentLine = #trees > 0 and talentLine(trees) or L.exportNoTalents,
...
Controller ruling 5 applies here: `Options.handle` keeps returning lines; only `Options.run` changes.
...
```lua
	it("prints each line through the chat prefix when the chat pref is on", function()
		...
		-- asked for chat as well. Controller ruling 5.
