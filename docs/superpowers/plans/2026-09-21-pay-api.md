...
- Produces (every later task reads these exact names):
  ...
  - `func (s *Store) StripeSubscriptionIDs(ctx) ([]string, error)` — every `stripe_subscription_id`
    whose row is still believed live (`source = 'stripe'`, `status != 'canceled'`). Added here,
    not in Task 8, so the `billing.Entitlements` interface (Task 4) — which Task 8's
    `Reconcile` also uses — can require it from the start without an ordering gap.
...
- [ ] **Step 4b: Write `StripeSubscriptionIDs` and its test**
...
Append to `store.go`, after `GuildBilling`:
...
Expected: PASS, every test including `TestStripeSubscriptionIDsListsOnlyNonCanceledStripeSourcedRows`.
...
- [ ] **Step 1: Confirm `StripeSubscriptionIDs` already exists (Task 2 added it)**

Task 2 already added `entitlements.Store.StripeSubscriptionIDs` (its own Step 4b) so that
Task 4's `billing.Entitlements` interface could require it with no ordering gap. Confirm it is
there before continuing:

```bash
cd /Users/jh/code/forever/.worktrees/pay-api/api
grep -n "func (s \*Store) StripeSubscriptionIDs" internal/entitlements/store.go
```

Expected: one match. If it is missing, stop and re-run Task 2's Step 4b before continuing —
every later step in this task depends on it.
