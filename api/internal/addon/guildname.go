...
// category C (Cc control, Cf format, Co private-use, Cs surrogate).
// Cf alone covers every exploit named in the second security review
// response (B): zero-width space (U+200B), zero-width joiner (U+200D),
// right-to-left override (U+202E), and the byte-order mark (U+FEFF) -
// none of them can appear in a real WoW guild name, and a bidi override
// in particular can make a guild's displayed name misleading about
// what it actually is. Go's standard library has no direct way to test
// whether a code point is *unassigned* in the current Unicode version
// (there is no Cn table); category C is the complete, precisely
// testable set the finding calls for.
