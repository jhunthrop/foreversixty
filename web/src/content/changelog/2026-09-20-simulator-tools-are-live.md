...
None of it is graded yet. [Spec support](/sim/specs) still reads "Not yet" for every damage spec — nothing has been checked against a real parse, because there are no real parses yet. And Droptimizer can only rank what the re-itemised raid tier actually has: a large share of the fork's own sourced item ids do not exist in the 1.60 client, so a boss can show fewer upgrades than it will once that tier is fully curated.
